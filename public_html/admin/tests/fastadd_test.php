...
$ttl = mysqli_query($f1db,
	"SELECT test.yr, test.name, sch.num
	FROM f1_test_schedule AS sch
	INNER JOIN f1_test AS test
	ON sch.test = test.no
	WHERE sch.no = $event_no
	LIMIT 1"
);
$ttl = mysqli_fetch_array($ttl);
$title = $ttl['yr'].' '.$ttl['name'].' - add test day '.$ttl['num'].' results';
...
if (isset($_POST['phase4'])) {
	...
	$active_drivers = mysqli_query($f1db, // Pilóta száma helyett az entr_no oszlop lesz hozzárendelve
		"SELECT test.no, driver.first, driver.de, driver.last, driver.sr
...
if (isset($_POST['phase5'])) {
	foreach ($_POST['entr_no'] as $key => $entr_no) {
		...
		$add = mysqli_query($f1db,
			"INSERT INTO f1_test_results(entr_no, session, place, tme, laps)
	...
	if (isset($_POST['make_visible'])) {
		...
		if ($vis) {
			echo 'Test day set visible<br>';
		}
		else {
			echo 'Test day couldn\'t set visible<br>';
		}
	}
	echo '<p><a href="/admin/test/'.$test_no.'/results/session/'.$event_no.'">View</a></p>';
}
